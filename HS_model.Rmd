...
3. Load Covariate data layers

3.1. Distance to Rivers, Roads & Woody Cover

3.1.1. Rivers
```{r}
# Calculate distance of each observation to nearest permanent river, then add column to dataframe
# rgeos::gDistance
distance.matrix.rivers <- gDistance(rivers, WBS,  byid=TRUE)
WBS@data$D_river<-apply(distance.matrix.rivers,1,min)
head(WBS@data)
...
```{r}
# Calculate the distance of each observation to the nearest primary road, then add the column to the dataframe
# rgeos::gDistance
distance.matrix.roads <- gDistance(roads, WBS,  byid=TRUE)
WBS@data$D_road<-apply(distance.matrix.roads,1,min)
head(WBS@data)
```

3.1.3. Woody Vegetation

```{r}
# Calculate distance of each observation to nearest patch of woody cover over 40%, then add column to dataframe
# rgeos::gDistance
distance.matrix.wood <- gDistance(woody, WBS,  byid=TRUE)
WBS@data$D_wood<-apply(distance.matrix.wood,1,min)
head(WBS@data)
