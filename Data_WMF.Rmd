...
2. Read data into project, adjust data structure and re-project where necessary
...
# project data - convert to SpatialPoints dataframe
coordinates(WB) <- ~x+y
proj4string(WB) <- crs
...
Layers provided by J.A. Stabach as used in Stabach (2015) and Stabach et al. (2016)
...
anthro1 <- raster::brick("Smithsonian_data/Data/Data_Layers/32736/reprojanth_risk_clp.tif")
TWI <- raster::brick("Smithsonian_data/Data/TWI/twi_200/hdr.adf")
TWI <- resample(TWI, anthro1)

```


2.2.4. Distance to Woody Vegetation

```{r}
D_wood <- raster("Smithsonian_data/Data/Data_Layers/32736/reprojwoody_dst_clp.tif")
```

2.2.5. Distance to Rivers

```{r}
D_river <- raster("Smithsonian_data/Data/Data_Layers/32736/reprojriver_dst_clp.tif")
```

2.2.6. Distance to Primary (T1) and Secondary (T5) Roads
